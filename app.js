...
const Mode = {
  ...
  detect(){
    ...
    if (/android/.test(ua)) return 'samsung';
    return 'laptop';
  },
  apply(mode){
    document.body.classList.remove('iphone','samsung','laptop');
    const effective = (mode==='auto') ? Mode.detect() : mode;
...
(function initModeUI(){
  ...
  document.querySelectorAll('.mode-btn').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      Mode.apply(m); mo.classList.add('hidden');
...
const BOffX = Math.floor((360 - (COLS*BW + (COLS-1)*BGap))/2);
const BOffY = 80;
...
let paddle = { x: 360/2 - G.paddleW/2, y: 640 - 36, w:G.paddleW, h:G.paddleH };
let bricks=[], drops=[], balls=[];
function newBall(stuck=true){ return { x: 360/2, y: paddle.y - G.ballR - 1, r:G.ballR, vx:0, vy:0, stuck }; }
...
function resetAll(){
  ...
  buildLevel(level); balls=[ newBall(true) ];
  updateHUD();
...
function movePaddleToClient(clientX){
  ...
  paddle.x = Math.max(8, Math.min(360 - paddle.w - 8, vx - paddle.w/2));
}
...
function step(dt){
  ...
  paddle.x = Math.max(8, Math.min(360 - paddle.w - 8, paddle.x));
  ...
  for (const b of [...balls]){
    ...
    if (b.x > 360 - b.r) { b.x = 360 - b.r; b.vx = -Math.abs(b.vx); }
    ...
    for (const row of bricks){
      for (const br of row){
        ...
        if (b.x + b.r > br.x && b.x - b.r < br.x + 30 && b.y + b.r > br.y && b.y - b.r < br.y + 16){
          const prevX = b.x - b.vx, prevY = b.y - b.vy;
          const fromLeft = prevX <= br.x, fromRight = prevX >= br.x + 30;
          const fromTop = prevY <= br.y, fromBottom = prevY >= br.y + 16;
          if ((fromLeft && b.vx>0) || (fromRight && b.vx<0)) b.vx *= -1; else b.vy *= -1;

          br.hp -= 1;
          if (br.hp <= 0){
            br.alive=false;
            score += (br.type===2?60 : br.type===3?80 : 50);
          } else {
            score += 20;
          }
          updateHUD();
...
// Render
function render(){
  ctx.fillStyle='#0e1429'; ctx.fillRect(0,0,360,640);
  ctx.fillStyle='#151b31'; ctx.fillRect(0,0,360,48);
  ...
  for (const row of bricks){
    for (const br of row){
      if (!br.alive) continue;
      const colors = ['#63e6ff','#4dabf7','#845ef7','#ffd43b','#ffa94d','#51cf66','#ff6b6b','#ced4da'];
      const c = colors[(Math.floor((br.y - 80)/(16+4)) % colors.length)];
      if (br.type===3){
        ctx.fillStyle='#98a6b3'; ctx.fillRect(br.x, br.y, 30, 16);
        ctx.fillStyle='rgba(255,255,255,.85)'; ctx.font='10px system-ui'; ctx.textAlign='center';
        ctx.fillText('🛡×'+br.hp, br.x + 15, br.y + 16 - 4);
        ctx.textAlign='left';
      } else {
        ctx.fillStyle=c; ctx.fillRect(br.x, br.y, 30, 16);
        if (br.type===2){ ctx.strokeStyle='rgba(0,0,0,.35)'; ctx.strokeRect(br.x+1, br.y+1, 28, 14); }
        ctx.fillStyle='rgba(255,255,255,.22)'; ctx.fillRect(br.x, br.y, 30, 4);
      }
    }
  }
  for (const d of drops){
